    ...
    def trainepoch(self, X, y, epoch_size=1):
        ...
        for _ in range(self.nepoch, self.nepoch + epoch_size):
            ...
            for i in range(0, len(X), self.batch_size):
                ...
                if self.cudaEfficient and use_cuda:
                    Xbatch = Xbatch.cuda()
                    ybatch = ybatch.cuda()
...
class MLP(PyTorchClassifier):
    def __init__(self, params, inputdim, nclasses, l2reg=0., batch_size=64,
        ...
        """
        self.nlayers = 0 if "nlayers" not in params else params["nlayers"]
        ...
        if params["nhid"] == 0:
            ...
            #         nn.Linear(params["nhid"], self.nclasses),
            #         )
            layers = [nn.Linear(self.inputdim,params["nhid"]),nn.ReLU(True)]
            for i in range(1,self.nlayers):
                layers = layers + [nn.Linear(params["nhid"],params["nhid"]),nn.ReLU(True)]
            layers = layers + [nn.Linear(params["nhid"],self.nclasses),nn.Dropout(p=self.dropout)]
